...
(function () {
  ...
  WGPU.preloadAudioWorklet = async function (opts) {
    ...
    if (!_saCtx.audioWorklet) {
      console.error('audio: no AudioWorklet on ' + location.origin +
  ...
  function stopAudioProducer() {
    ...
    if (_saGain) { try { _saGain.disconnect(); } catch (e) { /* ignore */ } _saGain = null; }
    if (_saNode) { try { _saNode.disconnect(); } catch (e) { /* ignore */ } _saNode = null; }
  }

  // Fade audio out, then tear down. Embedders that drive run/stop cycles
  // call this before starting the next run.
  WGPU.stopAudio = async function (fadeMs) {
    const ms = (typeof fadeMs === 'number') ? fadeMs : 50;
    if (_saCtx && _saGain) {
      const t = _saCtx.currentTime;
      try {
        _saGain.gain.cancelScheduledValues(t);
        _saGain.gain.setValueAtTime(_saGain.gain.value, t);
        _saGain.gain.linearRampToValueAtTime(0, t + ms / 1000);
      } catch (e) { /* ignore */ }
      await new Promise((res) => setTimeout(res, ms + 20));
    }
    stopAudioProducer();
    if (_saCtx) { await _saCtx.close().catch(() => {}); _saCtx = null; }
    _saReady = false;
  };
  ...
  const env = wrap({
    ...
    // Byte count for a path; -1 when unknown.
    ...
    __sys_exit: (code) => {
      console.error(`__sys_exit(${N(code)}): wasm requested process exit`);
      quitRequested = true;
      throw new Error(`__sys_exit(${N(code)})`);
    },
  });
  ...
  const saudio = wrap({
    saudio_js_init: (sample_rate, num_channels, buffer_size) => {
      ...
      // mid-interval.
      const pullFn = () => (exp && typeof exp._saudio_emsc_pull === 'function') ? exp._saudio_emsc_pull : null;
      const pullOne = () => {
        const pull = pullFn();
        if (!pull) { audioDebt = 0; return false; }
        // a starved fifo means no backlog is left, so the debt is paid:
        // keeping it would eat the front of the next sound queued
        const ptr = pull(bs); if (!ptr) { audioDebt = 0; return false; }
