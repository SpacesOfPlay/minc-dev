...
// Drop the producer and its output node, keeping the AudioContext and its
// registered worklet module.
function stopAudioProducer() {
    if (SOKOL._saudioProducer) { clearInterval(SOKOL._saudioProducer); SOKOL._saudioProducer = null; }
    if (SOKOL._saudioGain) { try { SOKOL._saudioGain.disconnect(); } catch (e) { /* ignore */ } SOKOL._saudioGain = null; }
    if (SOKOL._saudioNode) { try { SOKOL._saudioNode.disconnect(); } catch (e) { /* ignore */ } SOKOL._saudioNode = null; }
}
...
SOKOL.preloadAudioWorklet = async function(opts) {
    ...
    // AudioWorklet is secure-context only. https and localhost qualify.
    if (!SOKOL._saudioCtx.audioWorklet) {
        SOKOL.onLog('audio: no AudioWorklet on ' + location.origin +
...
// Exports of the running instance, or null once the run was stopped: async
// host callbacks (fetch responses) can land after SOKOL.stop().
const live = () => (SOKOL.instance ? SOKOL.instance.exports : null);
...
SOKOL.makeImports = function() {
    return {
        env: {
            ...
            // Byte count for a path; -1 when unknown. Lets file_read size
            // its buffer up front instead of growing it.
            __minc_file_size: (pathPtr) => {
                const data = SOKOL._vfs && SOKOL._vfs[readCStr(pathPtr)];
        ...
        fetch: {
            sfetch_js_send_head_request: v((slot_id, path_cstr) => {
                ...
                const fullDownload = () => {
                    ...
                    body.then((data) => {
                        const e = live(); if (e) e.sfetch_emsc_head_response(slot, BigInt(data.byteLength));
                    }).catch((err) => {
                        SOKOL._sfetchBodies.delete(path);
                        const e = live(); if (!e) return;
                        if (err && err.status) {
                            e.sfetch_emsc_failed_http_status(slot, BigInt(err.status));
                        } else {
                            console.error(`sokol_fetch: GET ${path} failed with: `, err);
                            e.sfetch_emsc_failed_other(slot);
                        }
                    });
                };
                ...
                if (cached) {
                    cached.then((data) => {
                        ...
                        const e = live(); if (e) e.sfetch_emsc_failed_other(slot);
                    });
                    return;
                }
                fetch(path, { method: 'HEAD' }).then((response) => {
                    ...
                    const e = live(); if (e) e.sfetch_emsc_head_response(slot, BigInt(len));
                }).catch(() => fullDownload());
            }),
            ...
            sfetch_js_send_get_request: v((slot_id, path_cstr, offset, bytes_to_read, buf_ptr, buf_size) => {
                ...
                const deliver = (bytes, rangeAdvance) => {
                    const e = live(); if (!e) return;
                    if (bytes.length > cap) {
                        e.sfetch_emsc_failed_buffer_too_small(slot);
                        return;
                    }
                    ...
                    e.sfetch_emsc_get_response(slot, BigInt(rangeAdvance), BigInt(bytes.length));
                };
                ...
                fetch(path, { method: 'GET', headers }).then((response) => {
                    if (!response.ok) {
                        const e = live(); if (e) e.sfetch_emsc_failed_http_status(slot, BigInt(response.status));
                        return;
                    }
        ...
        saudio: {
            saudio_js_init: (sample_rate, num_channels, buffer_size) => {
                ...
                // The producer outlives its instance if a run is swapped out
                // mid-interval.
                const pullFn = () => {
                    const e = SOKOL.instance && SOKOL.instance.exports;
                    return (e && typeof e._saudio_emsc_pull === 'function') ? e._saudio_emsc_pull : null;
                };
                const pullOne = () => {
                    const pull = pullFn();
                    if (!pull) { audioDebt = 0; return false; }
                    const ptr = pull(bs);
                    // nothing queued: the stream is caught up by definition,
                    // so drop the debt rather than eat the next sound
                    if (!ptr) { audioDebt = 0; return false; }
            ...
            saudio_js_shutdown: v(() => {
                ...
                if (SOKOL._saudioCtx) { SOKOL._saudioCtx.close().catch(() => {}); SOKOL._saudioCtx = null; }
                // the worklet module is registered on the context that just
                // closed; a later init has to add it to the new one
                SOKOL._saudioWorkletReady = false;
            }),
