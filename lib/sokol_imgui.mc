...
enum simgui_log_item_t {
    SIMGUI_LOGITEM_OK = 0,
    SIMGUI_LOGITEM_BUFFER_OVERFLOW = 1,
}
...
void simgui_setup(simgui_desc_t* desc) {
    ...
    _simgui.vbuf_size = cast(u64, _simgui.desc.max_vertices) * cast(u64, sizeof(ImDrawVert));
    _simgui.ibuf_size = cast(u64, _simgui.desc.max_vertices) * 3 * cast(u64, sizeof(ImDrawIdx));
...
u64 _simgui_roundup4(u64 val) {
    return val + 3 & ~cast(u64, 3);
}
}
...
void simgui_render() {
    ...
    _simgui_clear(&write_desc, cast(u64, sizeof(write_desc)));
    for i32 cl_index = 0; cl_index < draw_data.CmdLists.Size; cl_index++ {
        ...
        if vb_offset + vtx_size > _simgui.vbuf_size || ib_offset + idx_size > _simgui.ibuf_size {
    ...
    for i32 cl_index = 0; cl_index < cmd_list_count; cl_index++ {
        ImDrawList* cl = _simgui_imdrawlist_at(draw_data, cl_index);
        bind.vertex_buffer_offsets[0] = cast(i32, vb_offset);
        bind.index_buffer_offset = cast(i32, ib_offset);
        if tex_id != 0 {
            sg_apply_bindings(&bind);
        }
        u32 vtx_offset = 0;
        for i32 cmd_index = 0; cmd_index < cl.CmdBuffer.Size; cmd_index++ {
            ...
            if pcmd.UserCallback != null {
                ...
                if tex_id != cmd_tex_id || vtx_offset != pcmd.VtxOffset {
                    ...
                    sg_apply_uniforms(0, &sg_range{&vs_params, sizeof(vs_params)});
                    bind.vertex_buffer_offsets[0] = cast(i32, vb_offset + cast(u64, pcmd.VtxOffset * sizeof(ImDrawVert)));
                    sg_apply_bindings(&bind);
                }
